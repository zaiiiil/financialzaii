...
function maintMonthly(item) {
  const fv = parseFloat(item.freqValue) || 1;
  const fu = item.freqUnit || 'months';
  let mo;
  if (fu === 'month')  mo = 1;
  else if (fu === 'weeks') mo = (fv * 7) / 30.44;
  else mo = fv;
  return (item.cost || 0) / mo;
}
...
function renderMaintenance() {
  ...
  const totalMo  = maintenance.reduce((s, x) => s + maintMonthly(x), 0);
  ...
  if (sumEl) {
    sumEl.innerHTML = `
      <div class="kpi-grid" style="margin-bottom:1.2rem">
        <div class="kpi"><div class="kpi-lbl">Monthly cost (amortised)</div><div class="kpi-val" style="color:var(--red)">${fmt(totalMo)}</div></div>
        <div class="kpi"><div class="kpi-lbl">Annual cost</div><div class="kpi-val" style="color:var(--amber)">${fmt(totalYr)}</div></div>
        <div class="kpi"><div class="kpi-lbl">Items tracked</div><div class="kpi-val g-text">${maintenance.length}</div></div>
      </div>
      <div class="glass" style="padding:1rem 1.1rem;margin-bottom:1.2rem">
        <div style="display:flex;height:8px;border-radius:20px;overflow:hidden;gap:1px;margin-bottom:8px">
          ${maintenance.map((x,i) => {
            const w = totalMo > 0 ? Math.round(maintMonthly(x) / totalMo * 100) : 0;
        ...
        <div style="display:flex;flex-wrap:wrap;gap:10px">
          ${maintenance.map((x,i) => `<span style="font-size:11px;color:var(--t2);display:flex;align-items:center;gap:4px"><span style="width:8px;height:8px;border-radius:50%;background:${COLORS[i%COLORS.length]};display:inline-block"></span>${x.name} <strong style="color:var(--t)">${fmt(maintMonthly(x))}/mo</strong></span>`).join('')}
        </div>
      </div>`;
  }
  ...
  wrap.innerHTML = sorted.map((item, i) => {
    ...
    const yr = mo * 12;
    const fv = item.freqValue;
    const freqLabel = fu => fu === 'month' ? 'every month' : `every ${fv && fv > 1 ? fv + ' ' : ''}${item.freqUnit}`;
    return `<div class="glass" style="padding:1rem 1.1rem;margin-bottom:.6rem">
      <div style="display:flex;justify-content:space-between;align-items:flex-start;gap:8px;flex-wrap:wrap">
        <div style="flex:1;min-width:0">
          ...
          <div style="font-size:11px;color:var(--t3);margin-left:18px">${fmt(item.cost)} &nbsp;·&nbsp; ${freqLabel(item.freqUnit)}</div>
          ${item.notes ? `<div style="font-size:11px;color:var(--t2);margin-top:4px;margin-left:18px;font-style:italic">${item.notes}</div>` : ''}
        </div>
...
// ══════════════════════════════════════════════════════
// INSTALLMENTS & FIXED COSTS
// ══════════════════════════════════════════════════════
let editInstallId = null;

function installTotalMonths(item) {
  return item.total > 0 && item.monthlyAmt > 0 ? Math.ceil(item.total / item.monthlyAmt) : 0;
}
function installRemaining(item) {
  return Math.max(0, installTotalMonths(item) - (item.paidMonths || 0));
}
function installPaid(item) {
  return (item.paidMonths || 0) * (item.monthlyAmt || 0);
}
function installProgress(item) {
  if (!item.total || item.type === 'recurring') return 0;
  return Math.min(100, Math.round(installPaid(item) / item.total * 100));
}
function installPayoffDate(item) {
  ...
  const [y, m] = (item.startMonth || new Date().toISOString().slice(0,7)).split('-').map(Number);
  const d = new Date(y, m - 1 + (item.paidMonths || 0) + rem - 1, 1);
  return d.toLocaleDateString('en-US', { month: 'short', year: 'numeric' });
}

function renderInstallments() {
  const wrap = el('installments-list'); if (!wrap) return;
  const sumEl = el('installments-summary');

  const installs   = installments.filter(x => x.type !== 'recurring');
  const recurrings = installments.filter(x => x.type === 'recurring');
  const totalMo    = installments.reduce((s, x) => s + (x.monthlyAmt || 0), 0);
  const totalDebt  = installs.reduce((s, x) => s + Math.max(0, (x.total || 0) - installPaid(x)), 0);

  if (sumEl) {
    sumEl.innerHTML = installments.length ? `
      <div class="kpi-grid" style="margin-bottom:1.2rem">
        <div class="kpi"><div class="kpi-lbl">Total committed/mo</div><div class="kpi-val" style="color:var(--red)">${fmt(totalMo)}</div></div>
  ...
  if (installs.length) {
    html += `<div class="divl"><span>Installment payments</span></div>`;
    html += installs.map(item => {
      const prog = installProgress(item);
      const rem  = installRemaining(item);
      const paid = installPaid(item);
      const remaining = Math.max(0, (item.total || 0) - paid);
      const isDone = rem === 0;
      return `<div class="glass" style="padding:1rem 1.1rem;margin-bottom:.6rem${isDone?';opacity:.65':''}">
        <div style="display:flex;justify-content:space-between;align-items:flex-start;gap:8px;flex-wrap:wrap;margin-bottom:10px">
          ...
          <div style="display:flex;align-items:center;gap:12px;flex-shrink:0;flex-wrap:wrap">
            ...
            <div style="display:flex;gap:5px">
              ${!isDone ? `<button class="btn btn-g btn-sm" onclick="markInstallPaid(${item.id})" title="Mark one month paid">+1 month</button>` : ''}
  ...
  if (recurrings.length) {
    html += `<div class="divl"><span>Monthly recurring costs</span></div>`;
...
function initInstallmentListeners() {
  el('btn-add-install')?.addEventListener('click', () => {
    ...
    el('install-type').value = 'installment';
    toggleInstallFields('installment');
    openM('m-install');
  });

  el('install-type')?.addEventListener('change', e => toggleInstallFields(e.target.value));
  ...
  window.editInstall = id => {
    ...
    el('install-type').value = item.type || 'installment';
    toggleInstallFields(item.type || 'installment');
  ...
  window.markInstallPaid = async id => {
    const item = installments.find(x => x.id === id); if (!item) return;
    if ((item.paidMonths || 0) < installTotalMonths(item)) {
